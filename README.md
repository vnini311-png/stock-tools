...
| **Overview** | 本益比（手填）、MACD 柱狀值與單日變化、K 與 K−D（附黃交/死交）、券資比 P/C %、TAG（選單） |
...
TAG 是選單，選項來自筆記檔裡的一份標籤清單（`state.tags`），初次建立時是 `7m rev`。
選單最後兩項用來維護清單：

- **＋ 自訂…** —— 輸入新標籤，套用到這檔並加進清單，之後所有個股都選得到
- **⚙ 管理清單…** —— 一行一個編輯整份清單，刪掉整行就是移除該選項

選單只放在**個股 Review** 的表頭；**Overview 的 TAG 欄是唯讀**，直接顯示個股筆記裡的值。
若某檔的標籤已被移出清單，選單會顯示空白（原值仍在資料裡，重新選一次即覆蓋）。
...
2024 的舊筆記各自保留當初存的欄位名與值（I 段含融資、T 段含 RSI），不受這次調整影響。

### 存檔規則（重要）

> **編輯完一定要按「下載加密檔」**，覆蓋 `data/review.enc` 再 push。
> 頁面是靜態的、沒有後端，編輯只會即時存成瀏覽器裡的 localStorage 草稿；
> 沒下載成加密檔，資料就只活在這一台電腦的這一個瀏覽器裡。
> 有未下載的修改時，右上角會顯示「未儲存」，且「下載加密檔」按鈕會呼吸閃爍。

**草稿保護**：若上次編輯後沒下載就關掉，下次解鎖會偵測到本機草稿比加密檔新，
並詢問是否復原（附雙方筆數與最後編輯時間）；復原後仍標記為未儲存，不會二次遺失。
即使選了「取消」，前一版草稿也會備份在 `daily_review_v1_prev`，可用瀏覽器主控台取回：
