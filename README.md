...
只需補 **F**（基本面）與 **O**（總結）判斷。新筆記的 T 段四欄為 **MACD柱 / K / D / K−D**，
第一欄與 dashboard 同樣是柱狀值（DIF − 慢線），不是 DIF 本身。
2024 的舊筆記各自保留當初存的欄位名與值，仍會顯示 RSI，不受影響。編輯完按「下載加密檔」覆蓋 `data/review.enc` 再 push。
